...
app.controller('TaskUnfinishedController',['$scope','Api','$routeParams','$cookies', function ($scope, Api,$routeParams,$cookies) {

	//entfernt die Buttons falls kein Admin
	...
	.then(function(task){
		console.log(task)
		$("#description").val(task.data.properties.description);
		$("#text").val(task.data.properties.text);
		$("#sources").val(task.data.properties.sources);
	})
	.catch(function(){
		alert("Ups");
	})
